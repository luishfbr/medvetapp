...
export async function useAnimalDatabase() {
    ...
    async function searchAnimalByName(name: string, client_id: number) {
        try {
            ...
            const response = await database.getAllAsync<AnimalDatabase>(query, `%${name}%`, client_id);
            return response;
        } catch (error) {
            throw error;
        }
    ...
    async function removeAnimal(idAnimal: number) {
        try {
            await database.execAsync("DELETE FROM animals WHERE idAnimal = " + idAnimal)
        } catch (error) {
            throw error
        }
    }
    ...
    return { createAnimal, searchAnimalByName, updateAnimal, removeAnimal, showAnimals }
}
...
export async function useBudgetDatabase() {
    ...
    async function searchBudgetByDescription(description: string, animal_id: number) {
        try {
            const query = "SELECT * FROM budget WHERE description LIKE ? AND animal_id = ?";
            const response = await database.getAllAsync<BudgetDatabase>(query, `%${description}%`, animal_id);
            return response;
        } catch (error) {
            throw error;
        }
    ...
    async function removeBudget(idBudget: number) {
        try {
            await database.execAsync("DELETE FROM budget WHERE idBudget = " + idBudget)
        } catch (error) {
            throw error
        }
    }
    ...
    return { createBudget, updateBudget, removeBudget, showBudget, searchBudgetByDescription }
}